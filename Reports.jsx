...
export default function Reports() {
  ...
  /* ── Usage tracking ──────────────────────────────────────── */
  useEffect(() => {
    // Track page view with current route and active tab
    const payload = {
      event: 'reports_page_view',
      path: location.pathname,
      tab: activeTab,
      filters: { range, loc, dev, mode, status },
      timestamp: new Date().toISOString(),
    };
    if (typeof window.__trackUsage === 'function') {
      window.__trackUsage(payload);
    } else {
      // Fallback: log to console in non-prod environments
      if (process.env.NODE_ENV !== 'production') {
        console.debug('[Reports] usage:', payload);
      }
    }
  }, [location.pathname, activeTab, range, loc, dev, mode, status]);
